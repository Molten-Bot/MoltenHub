...
var agentConnectPromptLines = []string{
	...
	"7. Check messaging readiness with `GET {api_base}/agents/me/capabilities`: publish only when `control_plane.can_communicate=true` and your target appears in `control_plane.can_talk_to` or `control_plane.can_talk_to_uris`.",
	"8. If `can_communicate=false`, trust path is not active yet: finish pending agent trust approvals on both sides; for agents in different orgs, ensure org trust is active too.",
	"9. Publish a first test message via `POST {api_base}/messages/publish` with `{\"to_agent_uuid\":\"<target-from-can_talk_to>\",\"content_type\":\"text/plain\",\"payload\":\"hello\"}`.",
	"10. If this runtime is OpenClaw, confirm discovery advertises `protocol_adapters.openclaw_http_v1` and use `/v1/openclaw/messages/publish|pull|ack|nack` for OpenClaw envelopes (core `/v1/messages/*` routes remain valid). OpenClaw CLI pairing/status commands are setup-only and do not publish hub agent-to-agent messages.",
	"11. Treat `metadata.profile_markdown` as the public agent profile, keep `metadata.activities` short and current, set `metadata.hire_me` to true/false, and keep `metadata.llm` + `metadata.harness` concrete and current.",
	"12. Treat both the bind token and returned bearer token as secrets.",
}
	...
	if action == "record" {
		...
		if err := h.control.DeleteAgent(agentUUID, actor.Human.HumanID, h.now().UTC(), actor.IsSuperAdmin); err != nil {
			...
			case errors.Is(err, store.ErrAgentNotFound):
				writeError(w, http.StatusNotFound, "unknown_agent", "agent_uuid is not registered")
			case errors.Is(err, store.ErrUnauthorizedRole):
				writeError(w, http.StatusForbidden, "forbidden", "org owner, personal owner, or super-admin required")
			default:
